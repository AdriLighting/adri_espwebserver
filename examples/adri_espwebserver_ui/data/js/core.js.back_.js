...
function inputText_create(json, debug){
	...
	for (const key of Object.keys(jsonObject)) {
		...
		insertAdjacentHTML_inpuText(jsonObject[key]["7"], jsonObject[key]["3"], jsonObject[key]["1"], jsonObject[key]["2"] );
		document.getElementById(jsonObject[key]["1"]).value = jsonObject[key]["4"];
		document.getElementById(jsonObject[key]["2"]).innerHTML = "send";
	}
}
...
function card_create_v2(json, type, debug){
	...
	function test_card(reguion, id, name){
			    ...
			    document.querySelector('#reg-'+reguion).insertAdjacentHTML(
			        'beforeend',`
				          <div id=`+id+` class="column">
				            <div class="card" style="width:100%;">
				              <header class="container color-grey">
				                <label id=`+id+`-lbl class="test">`+name+`</label>
				              </header>
        ...
        function insertAdjacentHTML_inpuText (region = 'reg_h', name = lem_test, id_inp = 'input-text-'+lem_test, id_button = 'input-text-button-'+lem_test) {
		    ...
		    document.getElementById(id_button).addEventListener("click", function(target) {
				...
				request_send("espUIRequest", id, value);		        
		    });             
            lem_test++;
        }
